...
use cosmwasm_std::{
    from_binary, to_binary, Addr, Api, BankMsg, Binary, Coin, CosmosMsg, Deps, DepsMut, Env,
...
use crate::msg::{
    ConfigResponse, ExecuteMsg, InstantiateMsg, MigrateMsg, NftReceiveMsg, QueryMsg, StakingInfo,
};
use cw2::get_contract_version;
use cw20::{Balance, Cw20CoinVerified, Cw20ExecuteMsg, Cw20ReceiveMsg};
use cw721::Cw721ReceiveMsg;
use cw721_base::{
    msg::BatchMintMsg, msg::EditMsg, msg::ExecuteMsg as Cw721ExecuteMsg,
    msg::InstantiateMsg as Cw721InstantiateMsg, msg::MintMsg, msg::QueryMsg as Cw721QueryMsg,
    Extension,
};
...
use marble_collection::msg::{
    ConfigResponse as CollectionConfigResponse, ExecuteMsg as CollectionExecuteMsg,
    InstantiateMsg as CollectionInstantiateMsg, QueryMsg as CollectionQueryMsg,
};
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::UpdateEnabled { enabled } => {
            util::execute_update_enabled(deps.storage, info.sender, enabled)
        }
...
fn update_unclaimed_amount(
    ...
    let end_date: u64;
    if cfg.end_date > env.block.time.seconds() {
        end_date = env.block.time.seconds();
    } else {
        end_date = cfg.end_date;
    }

    if record.create_unstake_timestamp == 0u64 {
        record.unclaimed_amount += Uint128::from(
            (end_date / cfg.interval - record.last_timestamp / cfg.interval)
                * (record.token_ids.len() as u64),
        ) * cfg.daily_reward
            / Uint128::from(cfg.total_supply);
        record.last_timestamp = end_date;
...
pub fn execute_receive_nft(
    ...
    let collection_response: CollectionConfigResponse =
    ...
    if cfg.enabled == false {
        return Err(crate::ContractError::Disabled {});
    }
    if cfg.end_date < env.block.time.seconds() {
...
pub fn execute_claim(
    ...
    if util::get_token_amount(
        deps.querier,
        Denom::Cw20(cfg.cw20_address.clone()),
    ...
    let mut reward_msg = util::transfer_token_message(
        Denom::Cw20(cfg.cw20_address.clone()),
        staking_info.unclaimed_amount,
        info.sender.clone(),
    )?;
...
pub fn execute_create_unstake(
    ...
    STAKING.save(deps.storage, info.sender.clone(), &staking_info)?;
    CONFIG.update(deps.storage, |mut exists| -> StdResult<_> {
        exists.total_supply = cfg.total_supply - staking_info.token_ids.len() as u64;
        Ok(exists)
    })?;
    Ok(Response::new().add_attribute("action", "create_unstake"))
}
...
fn query_get_staking(deps: Deps, env: Env, address: Addr) -> StdResult<StakingInfo> {
    ...
    if cfg.end_date > env.block.time.seconds() {
        ...
        last_timestamp = cfg.end_date;
    }
    if staking_info.create_unstake_timestamp == 0u64 {
        staking_info.unclaimed_amount += Uint128::from(
            (last_timestamp / cfg.interval - staking_info.last_timestamp / cfg.interval)
                * (staking_info.token_ids.len() as u64),
        ) * cfg.daily_reward
            / Uint128::from(cfg.total_supply);
    }
    Ok(staking_info)
